...
_near_units = [];
{ if (player distance _x < 5) then {_near_units pushBack _x};} forEach playableUnits;
{
    if (!isNull _x && alive _x && player distance _x < 5 && !(_x isEqualTo player)) then {
        _rol = switch(side _x) do {case west: {"Policía"}; case civilian : {"Civil"}; case independent : {"EMS"};};
        _near lbAdd format ["????????? - %1", _rol];
        _near lbSetData [(lbSize _near)-1,str(_x)];
        _near_i lbAdd format ["????????? - %1", _rol];
...
ctrlSetText[2009,format ["Inventario: %1 / %2", life_carryWeight, life_maxWeight]];
