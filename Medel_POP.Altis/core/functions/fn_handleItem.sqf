...
_toUniform = [_this,5,false,[false]] call BIS_fnc_param; //Anulación manual para enviar artículos específicamente a un uniforme
_toVest = [_this,6,false,[false]] call BIS_fnc_param; //Anulación manual para enviar artículos específicamente a un chaleco
...
if (_bool) then {
    switch (_details select 6) do {
        ...
        case "CfgWeapons": {
            ...
            if (_isgun) then {
                if (!_ispack && _override) exitWith {}; //Estaba en el chaleco / uniforme, intenta cerrarlo para evitar que anule cosas ... (arma real y no un objeto)
                                ...
                                if (_override) then {
                                    ...
                                    if (_slotTaken) then {
                                        ...
                                        if (_action) then {
                                            ...
                                            player addItem _item; //Agregarlo a cualquier contenedor disponible
                                        };
                                    } else {
                                        switch (_type) do {
                                            case 1: {player addPrimaryWeaponItem _item;};
