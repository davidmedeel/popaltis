...
por: Icaruk
	...
	["borradatos"] call life_fnc_consola; // para borrar los datos de la consola que han sido guardados en profileNamespace
...
if (_param isEqualTo "menu") exitWith {
	...
	if (_oculto1 isEqualTo 1) then {
		["oculta", "izq", 1, true] call life_fnc_consola;
		_check1 cbSetChecked true;
	};
	if (_oculto2 isEqualTo 1) then {
		["oculta", "der", 1, true] call life_fnc_consola;
	...
	_e1 ctrlAddEventHandler ["SetFocus", {["mouseEnter", 0] call life_fnc_consola;}];
	_e2 ctrlAddEventHandler ["SetFocus", {["mouseEnter", 1] call life_fnc_consola;}];
	_e3 ctrlAddEventHandler ["SetFocus", {["mouseEnter", 2] call life_fnc_consola;}];
	_e4 ctrlAddEventHandler ["SetFocus", {["mouseEnter", 3] call life_fnc_consola;}];
	
	_e1 ctrlAddEventHandler ["Char", {["mouseEnter", 0] call life_fnc_consola;}];
	_e2 ctrlAddEventHandler ["Char", {["mouseEnter", 1] call life_fnc_consola;}];
	_e3 ctrlAddEventHandler ["Char", {["mouseEnter", 2] call life_fnc_consola;}];
	_e4 ctrlAddEventHandler ["Char", {["mouseEnter", 3] call life_fnc_consola;}];	
	
	_hb1 ctrlAddEventHandler ["MouseEnter", {["mouseEnter", 0] call life_fnc_consola;}];
	_hb2 ctrlAddEventHandler ["MouseEnter", {["mouseEnter", 1] call life_fnc_consola;}];
	_hb3 ctrlAddEventHandler ["MouseEnter", {["mouseEnter", 2] call life_fnc_consola;}];
	_hb4 ctrlAddEventHandler ["MouseEnter", {["mouseEnter", 3] call life_fnc_consola;}];
	
	// EHs para ocultar partes
	_check1 ctrlAddEventHandler ["CheckedChanged", {["oculta", "izq", _this select 1] call life_fnc_consola;}];
	_check2 ctrlAddEventHandler ["CheckedChanged", {["oculta", "der", _this select 1] call life_fnc_consola;}];
	
	// EH para botones
	_b1 ctrlAddEventHandler ["ButtonClick", {["exe", "loc"] call life_fnc_consola;}];
	_b2 ctrlAddEventHandler ["ButtonClick", {["exe", "srv"] call life_fnc_consola;}];
	_b3 ctrlAddEventHandler ["ButtonClick", {["exe", "glo"] call life_fnc_consola;}];
	_b4 ctrlAddEventHandler ["ButtonClick", {["borra"] spawn life_fnc_consola;}];
	
	// EH para keybinds
	_ui displayAddEventHandler ["KeyDown", {["tecla", _this] call life_fnc_consola;}];
	//_ui displayAddEventHandler ["KeyUp", {_this call des_fnc_keyBindUp}];
	
	// EH de salida
	_ui displayAddEventHandler ["Unload", {["unload"] call life_fnc_consola;}];
...
if (_param isEqualTo "tecla") exitWith {
	...
	if (_code in [2, 3, 4, 5]) then { // 1-4
		...
		if (_ctr AND !_shi AND !_alt) exitWith { // CTRL + 1-4
			["exe", "loc", _code - 2] call life_fnc_consola;
		};
		
		if (!_ctr AND !_shi AND _alt) exitWith { // ALT + 1-4
			["pagina", _code - 2] call life_fnc_consola;
		};	
		
		if (_ctr AND !_shi AND _alt) exitWith { // CTRL + ALT + 1-4
			["guardaPos", _code - 2] call life_fnc_consola;
			true
		};
		
		if (_ctr AND _shi AND !_alt) exitWith { // CTRL + SHIFT + 1-4
			["setPos", _code - 2] call life_fnc_consola;
			true
		};
	};
	...
	if (_code isEqualTo 30) then { // A
		if (_alt) exitWith { // ALT + A
			...
			["oculta", "izq", _bool] call life_fnc_consola;
		};
	};
	...
	if (_code isEqualTo 32) then { // D
		if (_alt) exitWith { // ALT + D
			...
			["oculta", "der", _bool] call life_fnc_consola;
		};	
	};
