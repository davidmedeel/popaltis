...
if ((cursorObject isKindOf "landVehicle") OR (cursorObject isKindOf "Ship") OR (cursorObject isKindOf "Air")) then {
	...
	["Admin Log",format ["%1 - (%2) ha llenado de gasolina un vehículo.",profileName,(getPlayerUID player)]] remoteexeccall ["medel_fnc_customlog",2];
} else {
    hint parsetext "<br/><t color='#e7ad3e' size='2' shadow='1'shadowColor='#7e521c'>ADMIN</t> <br/><t color='#e7ad3e' size='2'></t> <br/><t color='#FFFFFF' size='1.5'>Debes mirar a el vehículo que quieres llenar de gasolina.</t>";
};
