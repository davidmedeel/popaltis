...
if (isnil "uniforme") then {
	...
	if (_result) then {
		...
		["Admin Log",format ["%1 - (%2) se ha vestido de admin",profileName,(getPlayerUID player)]] remoteexeccall ["medel_fnc_customlog",2];
	};
} else {
	removeuniform player;
	player setunitloadout uniforme;
	uniforme = nil;
	["Admin Log",format ["%1 - (%2) se ha desvestido de admin",profileName,(getPlayerUID player)]] remoteexeccall ["medel_fnc_customlog",2];
};
