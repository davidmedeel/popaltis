...
if (isNil "modoAdmin") then {
	["Admin Log",format ["%1 - (%2) ha activado la función hojas",profileName,(getPlayerUID player)]] remoteexeccall ["medel_fnc_customlog",2];
	// LeavesFall, SmallFireBarrel
	player allowDamage false;
	modoAdmin = true;

	particulaCabeza = "#particlesource" createVehicle (getMarkerPos "");
	particulaCabeza setParticleClass "LeavesFall";
	particulaCabeza setParticleFire [0,0,0];
	particulaCabeza attachto [player, [0,0,0.15], "head"];

} else {
	["Admin Log",format ["%1 - (%2) ha desactivado la función hojas",profileName,(getPlayerUID player)]] remoteexeccall ["medel_fnc_customlog",2];
