...
[format["Estás pasando la frontera con otro nombre que no es el tuyo. Si quieres cambiarte el nombre, acude a nuestro TS3.",
_this select 1],
    ...
    case civilian: {
        ...
        if (LIFE_SETTINGS(getNumber,"save_civilian_position") isEqualTo 1) then {
            ...
            if (life_is_alive) then {
                if !(count life_civ_position isEqualTo 3) then {diag_log format ["[requestReceived] Mala posicion recibida. Datos: %1",life_civ_position];life_is_alive =false;};
