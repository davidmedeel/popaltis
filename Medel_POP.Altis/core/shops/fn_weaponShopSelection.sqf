...
if (isNull _control) exitWith {closeDialog 0;}; 
...
if ((uiNamespace getVariable ["Weapon_Shop_Filter",0]) isEqualTo 1) then {
    ...
    _item = CONTROL_DATAI(_control,_index);
    if (_price > CASH) then {
        _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Precio: <t color='#ff0000'>%1€</t><br/>Falta: <t color='#8cff9b'>%2€</t></t>",[(_price)] call life_fnc_numberText,[(_price - CASH)] call life_fnc_numberText];
    } else {
        _priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Precio: <t color='#8cff9b'>%1€</t></t>",[(_price)] call life_fnc_numberText];
    };
