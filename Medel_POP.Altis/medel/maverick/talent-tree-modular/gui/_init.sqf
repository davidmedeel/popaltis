...
missionNamespace setVariable [
	...
	compile preprocessFileLineNumbers "medel\maverick\talent-tree-modular\gui\_master.sqf"
];

{
	try {
		private _scriptLocation = getText (_x >> "scriptPath");
		if (_scriptLocation == "") throw format ["Dialog %1 no implementa scriptPath", configName _x];
		...
		[format ["[%1] %2", __filename, "GUI script registrado en variable " + format ["%1_script", configName _x]]] call BIS_fnc_log;
	} catch {
