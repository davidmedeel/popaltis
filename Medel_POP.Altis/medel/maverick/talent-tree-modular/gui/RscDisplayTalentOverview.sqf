...
try {
	...
	_updateGUI = {
		...
		if (isClass _nextLevelConfig) then {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1/%2 | SP: %3", life_currentExp - _currentLevelBottomExp, _currentLevelTopExp - _currentLevelBottomExp, life_currentPerkPoints];
			_ctrlProgress progressSetPosition _currentLevelProgress;
		} else {
			_ctrlTextStatusInfo ctrlSetText format ["Nivel Máximo | SP: %2", life_currentExp - _currentLevelBottomExp, life_currentPerkPoints];
	...
	switch (_mode) do
	{
		case "onLoad":
		{
			if (!life_perksInitialized) exitWith {
				hint "El servidor está cargando tus habilidades, vuelve a intentarlo más tarde";
			};
		...
		case "onTreeSelectionChanged":
		{
			...
			if (((count _limitedToFactions) == 0) || {(str playerSide) in _limitedToFactions}) then {
				if (_hasRequiredLevel) then {
					if (_hasParents) then {
						if (!_ownsPerk) then {
							if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > life_currentPerkPoints) then {
								_ctrlBtnPurchase ctrlSetText "SPS INSUFICIENTES";
								_ctrlBtnPurchase ctrlEnable false;
							} else {
								_ctrlBtnPurchase ctrlSetText "Desbloquear";
								_ctrlBtnPurchase ctrlEnable true;
							};
						} else {
							_ctrlBtnPurchase ctrlSetText "Desbloqueado";
							_ctrlBtnPurchase ctrlEnable false;
						};
					} else {
						_ctrlBtnPurchase ctrlSetText "Desbloquear";
					...
					_ctrlBtnPurchase ctrlSetText "NIVEL SUPERIOR REQ";
					_ctrlBtnPurchase ctrlEnable false;
				};
			} else {
				_ctrlBtnPurchase ctrlSetText "ROL INCORRECTO";
				_ctrlBtnPurchase ctrlEnable false;
			};
		};
		...
		case "onButtonPurchaseClick":
		{
			...
			if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > life_currentPerkPoints) exitWith {
				hint "¡No tienes suficientes SPS para desbloquear esta habilidad!";
			};
			
			if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > life_currentPerkPoints) then {
				hint "¡No tienes el nivel suficiente para desbloquear esta habilidad!";
			};
			...
			hint "Habilidad Desbloqueada";

			_ctrlBtnPurchase ctrlSetText "DESBLOQUEADA";
			_ctrlBtnPurchase ctrlEnable false;
