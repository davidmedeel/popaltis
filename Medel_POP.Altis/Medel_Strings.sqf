...
medel_StrLeft = {
	...
	_arr=[_in] call medel_StrToArray;
	_out="";
...
medel_StrIndex = {
	...
	_lh=[_hay] call medel_StrLen;
	_ln=[_ndl] call medel_StrLen;
	if (_lh < _ln) exitWith {-1};
	_arr=[_hay] call medel_StrToArray;
...
medel_StrInStr = {
	private["_out"];
	_in=_this select 0;
	_out=if (([_this select 0,_this select 1] call medel_StrIndex)==-1) then {false} else {true};
 	_out
};
...
medel_Replace = {
	...
	_old=_this select 1;
	_new=_this select 2;
	_na=[_new] call medel_StrToArray;
	_lo=[_old] call medel_StrLen;
	_ln=[_new] call medel_StrLen;
	_out="";
	for "_i" from 0 to (count _arr)-1 do {
		_tmp="";
		if (_i <= _la-_lo) then {
...
medel_Compare = {
	...
	_k=[_this,"CASE"] call medel_findFlag;
	_n=0;
	_s=0;
	for "_i" from 0 to 1 do {
		...
		switch (typeName _t) do {
			...
			case "STRING": {if !(_k) then {_this=[_this] call medel_ArrayToUpper}};
			default {_n=-1};
		};
	};
	_s1 = _this select 0;
...
medel_ArraySort = {
	...
	_a = +(_this select 0);
	_d = if ([_this,"DESC"] call medel_findFlag) then {-1} else {1};
	_k = if ([_this,"CASE"] call medel_findFlag) then {"CASE"} else {"nocase"};
	_s = -1;
	...
	for "_i" from 0 to (count _a)-1 do {
		...
		for [{_j=_i-1},{_j>=0},{_j=_j-1}] do {
			...
			_c=[_v2,_v1,_k] call medel_Compare;
			if (_c!=_d) exitWith {};
...
medel_findFlag = {
	...
	_arr=[_in] call medel_ArrayToUpper;
	_out=_flg in _arr;
	_out
};
	...
	if ((count _arr)>0) then {
		for "_i" from 0 to (count _in)-1 do {
			...
			if (typeName _as=="STRING") then {
				_aa = [_as] call medel_StrToArray;
				_p = _aa find ":";
				if (_p==_fl) then {
					if (([_as,_fl] call medel_StrLeft)==_flg) then {
						_org = _in select _i;
						_out=[_org,_p+1] call medel_StrMid;
					};
				};
			};
		};
		...
		for "_i" from 0 to (count _in)-1 do {
			...
			if (typeName _as=="STRING") then {
				...
				if (_p+1==(count _aa)-_fl) then {
					if (([_as,_p+1] call medel_StrMid)==_flg) then {
						_org = _in select _i;
						_out=[_org,_p] call medel_StrLeft;
					};
				};
			};
		};
