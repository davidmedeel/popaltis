...
if (!(count _queryResult isEqualTo 0)) exitWith {
    [1,"Ya existe una Banda con este nombre"] remoteexeccall ["life_fnc_broadcast",_ownerID];
...
if (!(count _queryResult isEqualTo 0)) exitWith {
    [1,"Ya estás en una Banda, abandonala para crear una nueva"] remoteexeccall ["life_fnc_broadcast",_ownerID];
    life_action_gangInUse = nil;
    _ownerID publicVariableClient "life_action_gangInUse";
};
